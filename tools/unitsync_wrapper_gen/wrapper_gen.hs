...
processTemplate funcs inFile = do
    let outFile = reverse $ tail $ dropWhile (/='.') $ reverse inFile -- strip ".template"
        header = ["// DO NOT EDIT: THIS FILE WAS GENERATED by unitsync wrapper generator",
                  "// from " <> B.pack inFile <> ". Edit that file instead."]
    putStrLn $ inFile ++ " -> " ++ outFile
    B.readFile inFile >>= B.writeFile outFile . B.unlines . (header++) . scan . B.lines
    where scan [] = []
...
functions = do
    r <- sepEndBy1 (fmap  Right (try function) <|> fmap Left (try comment) <?> "function definition or comment") $ skipMany newline
    eof
    return r
